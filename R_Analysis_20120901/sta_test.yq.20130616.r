...
FS_basic <- function(data, group, ID, name, method=c("StaTest", "ISA", "RF", "ENet", "SVMRFE", "SPLSDA"), kernel="linear"){
    ...
    if(method[1]){
        library(coin)
        parattest <- para_t.test(data, group, p.adj=c("none","bonferroni"), paired=FALSE, alternative="two.sided")$comb_info
...
para_t.test <- function(Data, group, p.adj=c("none","bonferroni"), ...){
    ...
    for(k in 1:length(p.adj)){
        ...
        ###  T-test  ###
        plist <- lapply(1:ncol(Data), function(x){try(pairwise.t.test(Data[,x], group, p.adj=method, ...)$p.value)})
...
para_chi.test <- function(Data, group, paired=FALSE, alternative="two.sided", conf.level=0.95){
	...
	result <- matrix(0,ncol(Data),(ngroup+ncol(col)))
	colnm <- paste(levels(group),"(existance)",sep="")
	for(i in 1:ncol(col)){
		...
		for(j in 1:ncol(Data)){
			...
			if(paired[i]==TRUE){
				result[j,(ngroup+i)] <- mcnemar.test(matrix(c(a,b,c,d),2,2),correct=TRUE)$p.value
			}
			else{
				result[j,(ngroup+i)] <- fisher.test(matrix(c(a,b,c,d),2,2),alternative=alternative)$p.value
			}
		}
	}
...
#	DF.3d <- plot_ordination(physeq, ordination, td.axes, type="split", color=col.type, shape=shape, justDF=TRUE)
	...
	result[["splitplot"]] <- ggplot(DF, aes_string(x=names(DF)[1], y=names(DF)[2], color=col.type, shape=shape, na.rm=TRUE)) + theme_bw() + 
					geom_point(aes(size=id.type), na.rm=TRUE) + scale_color_manual(values=color) + scale_size_manual("type", values=c(samples=5, taxa=2)) + 
...
#		result[["stressplot"]] <- stressplot
		#####  Biplot  #####
		result[["biplot"]] <- ggplot(DF, aes_string(x=names(DF)[1], y=names(DF)[2], color=col.type, shape=shape, na.rm=TRUE)) + theme_bw() + 
					geom_point(aes(size=id.type), na.rm=TRUE) + scale_size_manual("type", values=c(samples=5, taxa=2)) + 
