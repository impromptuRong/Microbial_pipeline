########  plot for PCA, COA
pcplot <- function(pca, group, color, filename, cbox=1, label=FALSE){
...
########  plot for NMDS
nmdsplot <- function(nmds, group, color, filename, cex=0.8, label=FALSE){
k <- ncol(nmds$points)
gof <- goodness(nmds)
...
denplot <- function(plotdata, group, color, filename){
	npic <- ncol(plotdata)
...
ldaplot <- function(plotdata, group, color, filename){
...
heatmaplot <- function(data, group, ID, color, method, filename){
...
hclusterplot <- function(dist, group, color, filename){
