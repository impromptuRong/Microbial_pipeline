...
foreach my $line(<RRDP>){
	...
	if(($genus ne "unclassified") && ($family eq "unclassified")){
		$family = $genus;
		$fscore = $gscore;
	}
	if(($family ne "unclassified") && ($order eq "unclassified")){
		$order = $family;
		$oscore = $fscore;
	}
	if(($order ne "unclassified") && ($class eq "unclassified")){
		$class = $order;
		$cscore = $oscore;
	}
	if(($class ne "unclassified") && ($phylum eq "unclassified")){
		$phylum = $class;
		$pscore = $cscore;
	}
